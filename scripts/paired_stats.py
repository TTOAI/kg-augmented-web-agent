"""Paired statistical tests for KG ablation evaluation (H1a + H1b).

Plan: `docs/kg_design/06_evaluation_protocol.md §4` + `08_contribution_scenarios.md`.

지원하는 테스트:
- **McNemar's exact test** (paired binary, H1a primary outcome)
- **Wilcoxon signed-rank test** (paired continuous, H1b token/step/time)
...
사용:
  # 2 variants (Baseline vs Full KG)
...
def render_report(
    ...
    lines = [
        "# Paired Statistical Tests — KG Ablation",
        "",
        "Generated by `scripts/paired_stats.py`. Reviewer-proof context: "
        "`docs/kg_design/06 §4` (dual H1, Bonferroni), "
        "`docs/kg_design/08_contribution_scenarios.md` (any-result-valuable framing).",
        "",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Paired statistical tests for KG ablation")
    parser.add_argument(
