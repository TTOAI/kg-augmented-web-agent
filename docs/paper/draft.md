...
실행 시점에서 KG는 세 모듈로 소비된다. 목표 페이지 클래스 추론기는 같은 태스크에 대해 LLM을 3번 호출한 뒤 가장 많이 일치한 답을 선택하는 자기 일관성(self-consistency) 방식으로, 에이전트가 도달해야 할 목표 페이지 클래스를 결정한다. 경로 탐색기는 현재 페이지 클래스에서 목표 페이지 클래스까지의 경로를, 정확한 경로가 없으면 인접 클래스·사이트 상위 진입점·전체 hub 순으로 점진적으로 후퇴하며 탐색한다. 힌트 생성기는 경로 단축 URL과 현재 페이지 표면 정보(액션 목록·필터 카테고리 존재)를 관측 메시지에 주입한다.
