...
실행 시점에서 KG는 세 모듈로 소비된다. target 클래스 추론기는 K=3 자기 일관성(self-consistency) LLM 호출로 태스크에서 에이전트가 도달해야 할 목표 페이지 클래스(이하 target 클래스)를 결정한다. 경로 탐색기는 6단계 cascade로 현재 페이지 클래스에서 target 클래스까지의 경로를 탐색한다. 힌트 생성기는 경로 단축 URL과 현재 페이지 표면 정보(액션 목록·필터 카테고리 존재)를 관측 메시지에 주입한다.
