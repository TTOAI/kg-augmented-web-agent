...
본 연구의 사전 측정 단계에서 KG가 구체값까지 한꺼번에 노출하면 에이전트가 그 정보를 검증 없이 따라 잘못된 경로로 빠지는 경우가 관측되었다. 이를 방지하기 위해 본 연구의 KG는 사이트의 구조 정보(어떤 페이지·경로·필터 카테고리가 존재하는지)까지만 힌트로 노출하고, 그 안에 들어갈 구체값(필터 라벨·URL 파라미터 값·텍스트 콘텐츠)은 에이전트가 페이지를 직접 보고 찾도록 한다.

### 2.3 측정 setup

WebArena-Verified GitLab 자가 호스팅 인스턴스를 사용한다. 에이전트 LLM은 OpenAI gpt-5.4-mini 단일 모델이다. 두 에이전트는 baseline (KG 미사용)과 KG 기반 에이전트 (KG 사용)이며 각 (태스크 × 에이전트) 셀에 3 시행을 측정한다. 측정 방법론은 round 단위 반복 측정 방식으로, round 시작 전에 (a) 선정 규칙(아직 관측이 없는 후보 셀 우선), (b) 종료 규칙(세 후보 H/L/Null에 ≥1 태스크 + 누적 태스크 ≤ 8), (c) 모든 결과 보고 의무를 git에 commit한다. 본 측정은 round 1·2 합계 7 태스크 × 2 에이전트 × 3 시행 = 42 시행이다.
