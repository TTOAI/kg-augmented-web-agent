...
WebArena-Verified GitLab 자가 호스팅 인스턴스를 사용한다. 에이전트 LLM은 OpenAI gpt-5.4-mini 단일 모델이다. baseline(KG 미사용)과 KG 기반 에이전트(KG 사용) 각각에 대해 각 태스크를 3회씩 반복 측정한다. 태스크는 한 번에 모두 정해두지 않고 회차별로 추가했으며, 회차마다 (a) 효과 후보(H)·한계 후보(L)·무영향 후보(Null) 중 아직 관측이 없는 후보를 우선 선정하고, (b) 세 후보 모두에 1개 이상 태스크가 관측되며 누적 태스크가 8개를 넘지 않을 때 종료하는 기준을 따랐다. 회차 1·2 합계 7개 태스크 × 2 에이전트 × 3회 = 총 42회 측정이다.
