...
본 논문은 두 질문을 묻는다. **KG는 어떤 조건에서 step을 줄이고, 어떤 조건에서 효과가 없거나 부정적인가? 그리고 그 효과는 task 통과율 변화로 이어지는가?** 이를 답하기 위해 본 연구는 ARIA 역할 속성·표준 폼 요소만으로 사이트 구조를 추출하는 site-agnostic KG 구축 프로토콜과, 추출된 구조를 *구조 단서 한정·구체값 미주입* 원칙(page class·path·filter category 같은 구조 단서만 노출하고 라벨 값·파라미터 값·텍스트 콘텐츠는 에이전트의 페이지 상호작용에 위임)으로 주입하는 최소 힌트 모드를 제시한다. 효과 가능성(H)·한계 가능성(L)·무영향 가능성(Null)의 archetype 매트릭스 coverage를 기준으로 선정한 7 과제를 측정 시작 *전* 가설 카드와 함께 git에 등록해 재현성을 확보했다. 측정 도중 WebArena-Verified 평가기의 메타데이터·strict-match·endpoint mismatch 결함이 KG 효과를 가리는 noise로 작용함을 발견하여, 평가기 출력 대신 *의미적 성공률*(에이전트가 task의 의미적 목표에 도달·완수했음을 로그·HAR로 검증한 trial 비율)을 별도 지표로 도입해 통과율과 step 효율을 분리 분석했다. 그 결과로 (a) 의미적 성공률은 V0=V1=6/7로 동일 — task 통과 여부는 baseline LLM 에이전트 능력에 좌우됨, (b) step 효율은 V1이 4 과제에서 평균 0.7~7.7 단축 + 2 과제에서 worst-case 표준편차 5~12배 감소, (c) 비ARIA 모달이 KG 적용 boundary 라는 세 관찰을 보고한다.
