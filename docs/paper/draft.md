...
실행 시점에서 KG는 세 모듈로 소비된다. target 클래스 추론기는 같은 태스크에 대해 LLM을 3번 호출한 뒤 가장 많이 일치한 답을 선택하는 자기 일관성(self-consistency) 방식으로, 에이전트가 도달해야 할 목표 페이지 클래스(이하 target 클래스)를 결정한다. 경로 탐색기는 6단계 cascade로 현재 페이지 클래스에서 target 클래스까지의 경로를 탐색한다. 힌트 생성기는 경로 단축 URL과 현재 페이지 표면 정보(액션 목록·필터 카테고리 존재)를 관측 메시지에 주입한다.

설계 원칙은 KG가 구조 정보만 노출하고 구체값은 에이전트가 페이지 상호작용에서 직접 찾도록 하는 것이다. 최소 힌트 방식은 (i) URL 파라미터 레시피, (ii) 페이지 클래스 라벨 자체, (iii) 필터 카테고리의 예시 값·옵션 값·기본 값을 모두 억제한다. 이 분리는 KG 기반 에이전트의 step 단축이 KG의 구조 정보 우월에서 오는지 KG가 정답을 흘려서 오는지 분리한다.

### 2.3 측정 setup

WebArena-Verified GitLab 자가 호스팅 인스턴스를 사용한다. 에이전트 LLM은 OpenAI gpt-5.4-mini 단일 모델이다. 두 에이전트는 baseline (KG 미사용)과 KG 기반 에이전트 (KG 사용, 최소 힌트 방식 적용)이며 각 (태스크 × 에이전트) 셀에 3 시행을 측정한다. 측정 방법론은 round 단위 반복 측정 방식으로, round 시작 전에 (a) 선정 규칙(아직 관측이 없는 후보 셀 우선), (b) 종료 규칙(세 후보 H/L/Null에 ≥1 태스크 + 누적 태스크 ≤ 8), (c) 모든 결과 보고 의무를 git에 commit한다. 본 측정은 round 1·2 합계 7 태스크 × 2 에이전트 × 3 시행 = 42 시행이다.
