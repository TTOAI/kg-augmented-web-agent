...
LLM 기반 웹 에이전트가 같은 사이트의 페이지 구조를 매번 관측에서 재발견하는 비효율을 해소하기 위해, 사이트 구조를 사전 계산된 지식 그래프(Knowledge Graph, KG)로 외부화하고 최소 힌트로 주입한 KG 기반 에이전트를 KG 미사용 baseline과 WebArena-Verified GitLab 7개 태스크 × 3 시행(trial)으로 비교했다. 측정 도중 평가기의 메타데이터·답 포맷·API 엔드포인트 불일치(mismatch) 결함이 다수 식별되어 평가기 통과율을 KG 효과 지표로 그대로 쓸 수 없었기에, 에이전트가 태스크의 의미적 목표에 도달·완수했는지를 로그·HAR로 검증한 시행 비율을 의미적 성공률로 재정의해 비교했다. 의미적 성공률은 baseline과 KG 기반 에이전트가 6/7(태스크 단위/task-level)로 동일하여 KG는 태스크 통과 여부를 바꾸지 않는 것으로 나타났으며, 통과 여부는 baseline LLM 에이전트의 일반 추론 능력에 좌우된다. step 효율 측면에서 KG 기반 에이전트는 의미적으로 성공한 6 태스크 중 4 태스크에서 평균 0.7~7.7 step을 단축했고, 특히 2 태스크(309, 664)에서는 baseline의 outlier trial(54·37 step)을 일관된 범위로 안정화해 표준편차를 5~12배 줄였다. 이러한 worst-case 표준편차 감소가 step 효율 개선의 주요 메커니즘이다. 비-ARIA 모달 단일 태스크(568)에서는 양쪽 모두 timeout 발생하여, KG의 ARIA 표준 기반 수집기가 비표준 컴포넌트를 포착하지 못함이 적용 한계로 드러났다. 결과적으로 KG는 step 효율 도구로서 유의미한 효과를 보이지만 의미적 성공률 자체에는 영향을 주지 않는다.
...
LLM 기반 웹 에이전트[1, 2]는 매 서브목표마다 (1) 어느 페이지 클래스가 태스크를 담당하는지, (2) 그 페이지에 어떤 필터·버튼이 있는지, (3) 어느 URL 경로가 단축인지를 관측만으로 재발견한다. 이 반복은 같은 사이트의 태스크마다 누적된다.
...
실행 시점에서 KG는 세 모듈로 소비된다. target 클래스 추론기는 K=3 자기 일관성(self-consistency) LLM 호출로 태스크에서 target 클래스를 결정한다. 경로 탐색기는 6단계 cascade로 현재 페이지 클래스에서 target 클래스까지의 경로를 탐색한다. 힌트 생성기는 경로 단축 URL과 현재 페이지 표면 정보(액션 카탈로그·필터 카테고리 존재)를 관측 메시지에 주입한다.

설계 원칙은 KG가 *구조 정보*만 노출하고 *구체값*은 에이전트가 페이지 상호작용에서 직접 찾도록 하는 것이다. 최소 힌트 모드(`KG_MODE=minimal`)는 (i) URL 파라미터 레시피, (ii) 페이지 클래스 라벨 자체, (iii) 필터 카테고리의 예시 값·옵션 값·기본 값을 모두 억제한다. 이 분리는 KG 기반 에이전트의 step 단축이 KG의 *구조 정보 우월*에서 오는지 *KG가 정답을 흘려서* 오는지 분리한다.

### 2.3 측정 setup

WebArena-Verified GitLab 자가 호스팅 인스턴스를 사용한다. 에이전트 LLM은 OpenAI `gpt-5.4-mini` 단일 모델이다. 두 에이전트는 baseline (KG 미사용, `KG_ENABLED=0`)과 KG 기반 에이전트 (KG 사용, `KG_ENABLED=1, KG_MODE=minimal`)이며 각 (태스크 × 에이전트) 셀에 3 시행을 측정한다. 측정 방법론은 round 단위 반복 측정 방식으로, round 시작 전에 (a) 선정 규칙(아직 관측이 없는 후보 셀 우선), (b) 종료 규칙(세 후보 H/L/Null에 ≥1 태스크 + 누적 태스크 ≤ 8), (c) 모든 결과 보고 의무를 git에 commit한다. 본 측정은 round 1·2 합계 7 태스크 × 2 에이전트 × 3 시행 = 42 시행이다.
...
102 — 안정적 평균 단축 (다중 hop NAV). KG는 `project/issue_list` 추론과 `/-/issues` 경로 단축을 활용해 baseline의 3-hop(프로젝트→Issues→라벨)을 1-hop `goto`로 압축한다. KG는 라벨 카테고리 존재만 알려주고 라벨 값 "help wanted"는 에이전트가 태스크에서 추출 → 드롭다운 직접 클릭. 양쪽 sd가 작아 효과가 시행 간 안정.
...
종합하면, KG는 (i) baseline이 outlier 분기 가능성이 있는 다중 단계 경로를 거치고 (ii) target 또는 의미적으로 인접한 클래스가 KG에 매핑되어 있을 때 평균 단축과 worst-case 표준편차 감소로 step 효율을 개선한다. baseline이 이미 효율적인 경우 KG는 무영향이거나 작은 역효과를 줄 수 있으며, 모든 효과는 step 단위에 한정되어 의미적 성공 여부 자체는 변화시키지 않는다 — 태스크 통과 능력은 baseline LLM 에이전트의 일반 추론 능력에 의해 결정된다.

### 3.5 태스크 568 — 비-ARIA 모달의 KG 적용 한계 (별도 분석)

다른 6 태스크와 달리 568은 양쪽 모두 의미적으로 실패한다. "Invite members" 다이얼로그가 GitLab Pajamas Vue 컴포넌트로 ARIA-non-conformant이며 KG의 `project/member_list` 클래스에 모달 내부 액션 카탈로그가 비어 있다. 두 에이전트 모두 모달 트리거(`Invite members` 버튼)까지는 도달하지만 모달 내부의 사용자 검색 입력·역할 선택을 처리하지 못해 3 시행 모두 20분 timeout 발생한다. KG 기여는 모달 진입 직전까지로 한정되며 내부에서 소멸한다. 이 한계는 KG가 ARIA 표준 기반 수집기로 빌드되어 있다는 의도적 설계 선택에서 기인하며, 비표준 컴포넌트가 다수인 최신 웹 프레임워크에서는 KG 적용 한계가 된다. 568은 다른 6 태스크와는 전혀 다른 패턴(둘 다 실패, KG 도움 범위 밖)이므로 §3.4의 효과 조건 분석에서 분리해 별도 절로 보고한다.
