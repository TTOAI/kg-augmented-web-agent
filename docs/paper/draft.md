...
KG가 답에 가까운 구체값까지 한꺼번에 제공하면 에이전트가 정보를 스스로 검증하지 않고 맹목적으로 따르는 경향이 있다. 따라서 본 연구의 KG는 사이트의 구조 정보(어떤 페이지·경로·필터 카테고리가 존재하는지)까지만 힌트로 노출하고, 그 안에 들어갈 구체값(필터 라벨·URL 파라미터 값·텍스트 콘텐츠)은 에이전트가 페이지를 직접 보고 찾도록 한다.
