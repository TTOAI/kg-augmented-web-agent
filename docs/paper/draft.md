...
세 단계로 수행한다. 1단계는 출발 URL 집합에서 너비 우선 탐색으로 페이지를 수집하고 URL 패턴(path 구조)으로 페이지 클래스를 할당한다. 2단계는 각 클래스에 속하는 페이지에서 ARIA 역할 속성(메뉴 항목, 옵션, 탭, 대화상자 팝업 트리거)과 표준 폼 요소만을 근거로 액션·필터·대화상자를 수집한다. 의도적으로 사이트와 무관하게 설계되어 비표준 컴포넌트는 포착하지 않는 대신 단일 구현으로 여러 사이트에 이식된다. 3단계는 URL 템플릿, 범위, 트리거 문구, 필터 카테고리를 결합해 페이지 클래스 목록을 만든다. 현 GitLab KG는 139 클래스·23 클래스의 필터 카테고리·2,731개의 페이지 간 이동 관계를 담는다.

### 2.2 실행 시점 통합과 최소 힌트

실행 시점에서 KG는 세 모듈로 소비된다. target 클래스 추론기는 K=3 자기 일관성(self-consistency) LLM 호출로 태스크에서 target 클래스를 결정한다. 경로 탐색기는 6단계 cascade로 현재 페이지 클래스에서 target 클래스까지의 경로를 탐색한다. 힌트 생성기는 경로 단축 URL과 현재 페이지 표면 정보(액션 목록·필터 카테고리 존재)를 관측 메시지에 주입한다.
