# Lab Report 009 — 과적합 제거, 코드 리팩토링, v2 아키텍처 설계

**날짜**: 2026-04-07~08  
**목적**: 008 안정성 재측정 결과의 regression 분석, 과적합 로직 제거, 코드 구조 개선, 다음 단계 아키텍처 설계.

---

## 008 재측정 결과 (Prior 도입 전 baseline)

| Task | 3회 결과 | 분류 |
|---|---|---|
| 44, 156, 357, 132, 205 | 3/3 PASS | 안정 |
| 339 | 2/3 | 불안정 |
| 45, 293, 169, 390 | 0/3 | regression |
| 258, 102, 308 | 0/3 | 기대 실패 (prior/벤치마크) |
...
- fill→click 리다이렉트가 MR 검색을 방해 (390)
- CSS locator 드롭다운 클릭이 필터/정렬 구분 못 함 (339, 45)
- 검색 경로 불안정 (293)
...
| NAVIGATE not_found → SUCCESS 변환 | 빈 결과 페이지 허용 | LLM 판단 덮어쓰기 |
| extract misuse 방어 | LLM의 goal_complete 오용 | 특정 LLM 패턴에 과적합 |
...
판단 대행 로직 제거:
- fill 드롭다운 감지 + click 리다이렉트 제거
- done 검증 LLM 호출 제거
- Executor는 순수 실행 + 결과 보고 레이어로
