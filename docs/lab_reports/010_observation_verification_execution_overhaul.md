...
- 타입 충돌 감지: 같은 이름이 여러 타입에 매칭되면 disambiguate 피드백 → LLM이 element_type으로 재시도

### 5. 검증 레이어 false positive/negative
...
### 8. HAR에 SPA 네비게이션 미기록
**증상**: pushState로 URL 변경 → HAR에 GET 요청 없음 → eval FAIL.  
...
3. **disambiguate**: 같은 이름의 요소가 여러 타입에 존재하면 LLM에게 되물음

### 검증 전략
- **결정론적 검증 제거**: false positive/negative 반복 → LLM 자가 판단이 더 정확
...
| + disambiguate + element_type | **PASS** | **57s/31** | **Search 버튼 정확 클릭** |
...
| 파일 | 변경 |
|---|---|
| `runtime/executor.py` | DOM 안정화, nearby 추출, disambiguate, element_type, navigation URL 강제, 검증 제거, extract 제한, retry/replan/budget 조정 |
...
| `benchmarks/.../adapter.py` | NAVIGATE SUCCESS 후 HAR 리로드 |
| `tests/test_runtime_llm.py` | 실패 테스트 기대값 수정 (goal 실패 → NOT_FOUND_ERROR) |

---

## 다음 단계
- 안정 태스크 regression 확인 (44, 156, 357, 132, 205)
- 전체 측정 재실행
- 339 반복 안정성 확인 (3회)
