...
def build_system_prompt(prior_bundle: PriorBundle | None) -> str:
    ...
    lines = [
        ...
        "",
        "Actions:",
        '  "click"     — click a link or button; set "target" to the visible name.',
        '               If ambiguous, set "url" (pathname) or "element_type" ("button"/"link") to disambiguate.',
        '  "fill"      — type into a field; set "target" (field name), "value" (text). Set "submit": true to press Enter.',
        '  "goto"      — navigate to a URL; set "url"',
        '  "goback"    — go back to the previous page',
        '  "search"    — search; set "target" (query)',
        '  "observe"   — get filtered observation when lists are truncated; set "target" (keyword)',
        '  "extract"   — data is visible; set "value" (the complete answer, no extra words), "label"',
        '  "done"      — task is complete',
        '  "not_found" / "permission_denied" / "action_not_allowed" / "unknown_error" — failure actions',
    ]
