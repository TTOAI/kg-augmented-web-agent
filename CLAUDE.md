...
**Runtime layer** (`site_adaptive_webagent/runtime/`): Site-adaptive prior knowledge and execution routing — not wired to the agent yet, schema only.
- `types.py` — dataclasses for `SiteProfile`, `PageType`, `ActionSchema`, `WorkflowHint`, `ValidatorRule`, `PolicyRule`, `FailurePattern`, `PriorBundle`, and execution records (`TaskRun`, `StepRecord`, etc.).
